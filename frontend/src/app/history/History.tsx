...
import type { CampaignHistoryLog } from '@/types';
import { toast } from 'sonner';
import { motion } from 'framer-motion';
import { EmptyState } from '@/components/ui/empty-state';

export function History() {
  const [logs, setLogs] = useState<CampaignHistoryLog[]>([]);
                  ...
                  filteredLogs.map((log) => (
                    <TableRow key={log.id} className="hover:bg-muted/30 transition-colors group">
                      ...
                      <TableCell className="text-center">
                        <Badge variant={log.status.toLowerCase() as any} className="capitalize">{log.status}</Badge>
                      </TableCell>
